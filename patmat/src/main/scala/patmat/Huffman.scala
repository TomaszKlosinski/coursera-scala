...
object Huffman {
   ...
   *   }
   */
    def times(chars: List[Char]): List[(Char, Int)] = (for (char <- chars) yield (char, chars.count(_ == char))).distinct
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = for (charFreq <- freqs.sortBy(_._2)) yield Leaf(charFreq._1, charFreq._2)
